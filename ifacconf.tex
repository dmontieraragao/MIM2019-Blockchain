...
\title{Beer Game over Blockchain\thanksref{footnoteinfo}} 
...
\section{Fundamentac\~{a}o Teorica}

Next we see a few subsections.

\subsection{Blockchain}

\cite{lindell2014introduction} define criptografia como o estudo de técnicas matemáticas para assegurar informações digitais, sistemas e computação distribuída contra ataques adversários.

Uma técnica importantíssima para entender blockchain é a função hash criptográfica. Ela é uma função matemática, H(x) = y, que pega uma entrada x de tamanho qualquer e produz uma saída y, conhecida como digest, de comprimento fixo em tempo razoável. Porém, as características mais importantes de tais funções para a blockchain são:
\begin{enumerate}
    \item Resistência à colisão: dadas duas entradas distintas, x e y, será impossível encontrar o mesmo digest, H(x)$\neq$H(y).
    \item Hiding: é impossível encontrar a entrada da função a partir de um digest.
    \item Amigáveis a desafios: dados y e r, um número aleatório gerado a partir de uma distribuição de baixa entropia, a melhor maneira de encontrar x tal que H(r$\mid$x) = y é através de advinhações aleatórias.
\end{enumerate}

Caso não seja especificada uma função hash a ser utilizada, a padrão é a sha-256 \cite{U.S.DepartmentofCommerce:2012:SHS:2408113}.

Para construir a blockchain necessitamos de um ponteiro hash, que contém o hash da informação armazenada e um ponteiro que indica onde está a informação. O ponteiro é utilizado para recuperar os dados, e o hash é utilizado para verificar se a informação não sofreu adulteração.

Blockchain é uma estrutura de dados que consiste em uma lista encadeada \cite{Cormen:2009:IAT:1614191} através de ponteiros hash. Cada elemento da lista é um bloco de conteúdo que possui um ponteiro para o bloco anterior bem como um hash deste bloco anterior. A cabeça consiste em um ponteiro hash para o bloco mais recente. E no início é criado um bloco gênese, que marca o início da cadeia de blocos.

\begin{figure}[hpb]
\begin{center}
\includegraphics[width=8.4cm]{figs/blockchain-exemplo.png}    % The printed column width is 8.4 cm.
\caption{Exemplo Blockchain.} 
\label{fig:blockchain}
\end{center}
\end{figure}

A grande vantagem dessa estrutura é a grande resistência à fraude. Caso um bloco da cadeia seja alterado, é possível detectar a alteração com o hash armazenado no bloco seguinte da cadeia, pois ele contém o hash do bloco antes da fraude. Portanto, uma fraude necessita alterar toda a cadeia a partir do bloco fraudado. Porém, tal alteração é inviável computacionalmente e economicamente, já que gastaria muito recurso energético relacionado as novas computações necessárias \cite{nakamoto2012bitcoin}.

As informações armazenadas nos blocos da cadeia são as transações entre os usuários. A questão importante relativa as transações é: como provar que uma transação foi realmente realizada por um usuário e não seja possível negar a autoria? Para isso usa-se uma técnica criptográfica conhecida como assinatura digital \cite{Johnson:2001:ECD:2701775.2701951}\cite{lindell2014introduction}.

Uma assinatura digital tem as mesmas operações de uma assinatura manual, a diferença é que a escrita, leitura e verificação de autenticidade é toda digital. As propriedades de uma assinatura digital são:
\begin{enumerate}
    \item Se um usuário X assinou a transação, somente X é capaz de utilizar aquela assinatura e qualquer outro usuário é capaz de verificar facilmente que a assinatura é de X.
    \item Cada assinatura gera um forte acoplamento ao que foi assinado, de forma que uma assinatura de X em uma transação não pode ser utilizada para provar outra transação.
\end{enumerate}

Felizmente, essas propriedades são possíveis graças a esquemas de criptografias de chave pública \cite{lindell2014introduction}.

\begin{figure}[hpb]
\begin{center}
\includegraphics[width=8.4cm]{figs/transacoes-exemplo.png}    % The printed column width is 8.4 cm.
\caption{Exemplo transação.} 
\label{fig:blockchain}
\end{center}
\end{figure}

Portanto, uma blockchain é uma estrutura ideal para o registro resistente à fraude de transações e também a irrefutabilidade de transações, o que permite rastrear informações sobre transações de maneira segura.
